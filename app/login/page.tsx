...
import Password from './visibility_component'
import { useRouter } from "next/navigation";

const PasswordComponent = Password as unknown as React.ComponentType<{ value: string; onChange: (val: string) => void }>;
...
export default function Login() {
    ...
    const HandleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!id.trim() || !pass.trim()) {
            ...
            setError("");
            console.log(`${id},${pass} Login success`);
    ...
    return (
        <div>
            <div className="relative flex h-screen w-full flex-col items-center justify-center overflow-hidden px-6 pixel-bg text-white selection:bg-purple-500/30 font-display">
                {/* Background Elements */}
                ...
                {/* Glass Card */}
                <div className="glass-card w-[90%] sm:w-[85%] md:w-full max-w-[400px]  p-5 sm:p-8  relative z-10 shadow-2xl top-6">
                    ...
                    <div className="text-center  mb-6 sm:mb-10">
                        <h1 className=" text-xl sm:text-2xl md:text-3xl font-pixel text-white mb-4 leading-relaxed tracking-wider" style={{ textShadow: "4px 4px 0 #4c1d95" }}>GAME VERSE</h1>
                    ...
                    <form onSubmit={HandleSubmit} className="space-y-6">
                        ...
                        <PasswordComponent
                            value={pass}
                            onChange={(val: string) => setPass(val)}
                        />


                        {error && (
                            <p className="text-white-400 text-sm p-1">{error}</p>
                        )}

                        <div className="pt-6">

                            <button className="arena-btn w-full h-14 flex items-center justify-center gap-3 text-white font-pixel text-sm tracking-wide group relative overflow-hidden" type="submit">
                                <span className="relative z-10 lg:text-base sm:text-xs pr-0.5">ENTER THE ARENA</span>
